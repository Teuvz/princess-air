package objects.platformer
{
	 ...
	 */	
	public class PrincessPhysics extends CitrusObject implements ISpriteView
	{
		/**
		...
		public var _inverted:Boolean = false;
		...
		public static function Make(name:String, x:Number, y:Number, width:Number, height:Number, view:*):PrincessPhysics
		{
			if (!view)
				view = MovieClip;
			return new PrincessPhysics(name, { x: x, y: y, width: width, height: height, view: view } );
		}
		 ...
		 * the PhysicsObject.</p>
		 */		
		public function PrincessPhysics(name:String, params:Object=null)
		{
			...
			if (!_box2D)
			{
				throw new Error("Cannot create PrincessPhysics when a Box2D object has not been added to the state.");
				return;
			}
